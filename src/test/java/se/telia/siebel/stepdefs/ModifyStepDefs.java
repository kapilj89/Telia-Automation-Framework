...
public class ModifyStepDefs implements En {
		...
		And("^call ModifyAssetToQuote for ServiceBundle \"([^\"]*)\" from Package \"([^\"]*)\" to \"([^\"]*)\"",
				(String ServiceBundle, String ExistingSpeed, String ModifiedSpeed) -> {
					try {
						...
						String[] Bundles = ExcelData.getData(CommonStepDefs.Tc_Name, ServiceBundle).split(";");
						String[] CurrentSpeed = ExcelData.getData(CommonStepDefs.Tc_Name, ExistingSpeed).split(";");
						String[] NewSpeed = ExcelData.getData(CommonStepDefs.Tc_Name, ModifiedSpeed).split(";");
				...
				(String ServiceBundle, String RelationshipName, String BasePackage, String NewPackage) -> {
					try {
						...
						List<String> Products = new ArrayList<String>(
								Arrays.asList(ExcelData.getData(CommonStepDefs.Tc_Name, ServiceBundle).split(";")));
						List<String> Realtions = new ArrayList<String>(
								Arrays.asList(ExcelData.getData(CommonStepDefs.Tc_Name, RelationshipName).split(";")));
						List<String> ExsitingSpeed = new ArrayList<String>(
								Arrays.asList(ExcelData.getData(CommonStepDefs.Tc_Name, BasePackage).split(";")));
						List<String> NewSpeed = new ArrayList<String>(
								Arrays.asList(ExcelData.getData(CommonStepDefs.Tc_Name, NewPackage).split(";")));
	...
	/** --- this step to be checked functionally and to be corrected Line 223 **/
		
		When("^call SynchronizeQuote and add ExtendCommitment \"([^\"]*)\" Modify order$", (String Package) -> {
			Quote quote = dataStorage.getQuote();
			...
			quoteItemList.stream()
					.filter(quoteItem -> !"Penalty PS".equalsIgnoreCase(quoteItem.getFulfillmentItemCode()))
					.forEach(quoteItem -> {
							if (quoteItem.getName().equalsIgnoreCase("Service Bundle-TV-IPTV-1704")) {
								quoteItem.setTSCommitmentReason("Extend Commitment");
										System.out.println("COMMITMENT EXTENDED");
