	...
	public MoveStepDefs(DataStorage dataStorage) {
		...
		And("^call QuerySelfServiceAccount to add end date$", () -> {
		  ...
		  When("^call QueryCustomer using SSN \"([^\"]*)\" to get PrimaryAddressId$", (String ssn) -> {
	            ...
	            String primaryAddressId = queryCustomer.getPrimaryAddressId(ExcelData.getData(CommonStepDefs.Tc_Name, ssn));
		...
		When("^call SynchronizeQuote for \"([^\"]*)\" Move order$", (String Stream) -> {
			...
			quoteItemList.stream()
					...
					.forEach(quoteItem -> {
							try {
							if (ExcelData.getData(CommonStepDefs.Tc_Name, Stream).equalsIgnoreCase("xDSL")) {
		...
		And("^check \"([^\"]*)\" and Move all exsiting Asset with help of product item name \"([^\"]*)\" RelationShipName \"([^\"]*)\" and Package \"([^\"]*)\" to \"([^\"]*)\"$",
				(String flag, String ServiceBundle, String RelationshipName, String BasePackage, String NewPackage) -> {
					...
					if (ExcelData.getData(CommonStepDefs.Tc_Name, flag).equalsIgnoreCase("N")) {
						System.out.println("\nQueryQuote\n");
						QueryQuote queryQuote = new QueryQuote(dataStorage);
						...
						List<String> Products = new ArrayList<String>(Arrays.asList(ExcelData.getData(CommonStepDefs.Tc_Name, ServiceBundle).split(";")));
						List<String> Realtions = new ArrayList<String>(Arrays.asList(ExcelData.getData(CommonStepDefs.Tc_Name, RelationshipName).split(";")));
						List<String> ExsitingSpeed = new ArrayList<String>(Arrays.asList(ExcelData.getData(CommonStepDefs.Tc_Name, BasePackage).split(";")));
						List<String> NewSpeed = new ArrayList<String>(Arrays.asList(ExcelData.getData(CommonStepDefs.Tc_Name, NewPackage).split(";")));
